...
/*	//上传文档、文献切换
	...
	$("#LEFT,#RIGHT").click(function(){
		...
		if(docFlag == false){
			if(headerId != "LEFT"){
				$(this).removeClass('bg_lg').addClass('bg_bg');
				$('#LEFT').removeClass('bg_bg').addClass('bg_lg');
				$('#filePageOne').slideUp('fast',function(){
					$('#filePageTwo').hide();
					$('#filePageDoc').slideDown('fast');
					docFlag = true;
				});
			}
		}
			...
			if(headerId != "RIGHT"){
				$(this).removeClass('bg_lg').addClass('bg_bg');
				$('#RIGHT').removeClass('bg_bg').addClass('bg_lg');
				$('#filePageDoc').slideUp('fast',function(){
					$('#filePageOne').slideDown('fast');
					docFlag = false;
				});
			}
		}
	...
	var URL = "/dmis/Doc/getUploadRecoder";
	var pageBar = "upPageBar";
	var listContent = "uploadR";
	var listHTML="uploadHTML";
	var PASS = '通过';
	var noPASS = '未通过';
	$("a[data-toggle='tab']").click(function(){
		
		var enable =  $(this).parent().attr('class');
		var getContentId = this.id;
		if(enable == 'active')
			return false;
		else{
			...
			switch(getContentId){
					case 'upload':
						...
						break;
					case 'download':
						URL = "/dmis//DownloadRecoder/getDownloadRecoder";
						pageBar = "downPageBar"; 
						listContent = "downloadR";
						listHTML = "downloadHTML";
						getListPanel(URL,1,pageBar,listContent,listHTML);
						break;
					case 'favo':
						URL = "/dmis/Favorites/getMyFavorites";
						pageBar = "myfavoritesBar"; 
						listContent = "myfavoritesR";
						listHTML = "favoHTML";
						getListPanel(URL,1,pageBar,listContent,listHTML);
						break;
					case 'notice':
						URL = "/dmis/Notice/getNotices";
						pageBar = "noticeBar"; 
						listContent = "noticeR";
						listHTML = "noticeHTML";
						getListPanel(URL,1,pageBar,listContent,listHTML);
	...
	// 页面初始化加载
	getListPanel("/dmis/Doc/getUploadRecoder",1,"upPageBar","uploadR","uploadHTML");
