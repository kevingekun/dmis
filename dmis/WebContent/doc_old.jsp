...
<div class="widget-box">
          <div class="widget-title">
			...
			<div class="widget-radio">
				<input id="fileEdit1" class="btn btn-small" type="button" value="新增文档" style="margin-bottom:5px">
				<input id="fileEdit2" class="btn btn-small" type="button" value="新增文献" style="margin-bottom:5px">
...
<div id="infoEdit" style="display:none;" align="center">
 <div id="fileEditContent1" style="display:none;" >
	<div class="infoHeader bg_lg">上传文档</div><br>
         <form id="uploadDocDocupload"  action="Doc/uploadDocDoc" method="post" enctype="multipart/form-data">
       		...
       		<div class="control-group">
                    <div class="controls">
                    	 ...
                    	 <div class="main_input_box">
                             ...
                             <select name="doc.language" style="width:100px;">
                             	<option> 选择语言 </option>
                                <option value="1"> 中文  </option>
                                <option value="0"> 英文  </option>
                             </select>
                             <select name="doc.level" style="width:100px;">
                             	<option> 选择级别 </option>
                         ...
                         <div class="main_input_box" id="docType" style="display: none" >
                          ...
                          <input type="hidden" id="tIdDoc" value="-1"/><!--设置一个隐藏的input,用于存中间值-->
                            ...
                            <span class="add-on bg_lg" ><li class="icon-file"></li></span><input type="text" id="txt" style="width:195px;">	<span class="add-on bg_ic" onclick="f.click()"><a style="color:#2E363F;" href="javascript:void(0);">浏览</a></span>
                       		...
                       		<br><input type="file" id="f" onchange="txt.value=this.value" name="uploadFile" class="files" onblur="checkType('file')" />
 ...
 <div id="fileEditContent2" style="display:none;">
      ...
      <form id="uploadDocform"action="Doc/uploadDoc" method="post" enctype="multipart/form-data"> 
         ...
         <div class="row" id="filePageTwo" style="color:#FFF; font-size:14px;margin:0 0 0 15px; display:none">
       		<div class="control-group">
                    ...
                    <div class="main_input_box">
                    	 ...
                    	 <div class="main_input_box">
                            <span class="add-on bg_lg" ><li class="icon-time"></li></span><input style="width:45%" type="date" placeholder="请输入出版（发表）时间"name="publishedTime" />
                        </div><br/>
                         <div class="main_input_box">
                            <span class="add-on bg_lg" ><li class="icon-file"></li></span><input type="text" id="txt" style="width:195px;">	<span class="add-on bg_ic" onclick="f.click()"><a style="color:#2E363F;" href="javascript:void(0);">浏览</a></span>
              ...
              <center><span style="color:#FFF" id="passTips">提示信息</span></center>
          </div> 
         </form> 
      </div>
 </div>
          ...
          <div class="widget-content ">
           <s:form name="form2" id="form2" method="post">
            <table class="table table-bordered table-striped with-check">
              <thead>
                <tr>
                  <th style="width:20px;"><input type="checkbox" style="margin-bottom:4px"  id="selAll"  onclick="selectAll();"/><input type="hidden" id="checkselect" value="false"></th>
              ...
              <s:iterator value="#request.page.pageList" >
                <tr>
                  <td ><input type="checkbox" style="margin-left:12px" name="checkAll" id="checkAll"  value="<s:property value="id"/>" onClick="setSelectAll();"/></td>
                  ...
                  <td ><a class="btn btn-mini" href="Doc/delete?id=<s:property value="id"/>&pageNo=<s:property value="#request.page.pageNo"/>" id="updateDoc">删除</a>
                  <s:if test="!isPass">
                  <a class="btn btn-mini" href="Doc/passDoc.action?id=<s:property value="id"/>&pageNo=<s:property value="#request.page.pageNo"/>">通过</a>
                  </s:if>
                  <s:else>
                  </s:else>
                  </td>
            ...
            <div class="pagination alternate" style="float:left;margin-top:30px;margin-left:-15px">
				              <ul>
				              	<s:if test="#request.page.showPrv != 0">
					              	<li class id="firstPage" data-id="PAGE"><a href="Doc/list?pageNo=1&state=${state}">首页</a></li>
					                <li class id="lastPage" data-id="PAGE"><a href="Doc/list?pageNo=<s:property value="#request.page.pageNo -1"/>&state=${state}">上一页</a></li>
					            </s:if>
					            <s:else>
					            	...
					            	<s:iterator value="#request.page.pageNum" id="key">
						            	<s:if test="#key != #request.page.pageNo">
						            		<li data-vl="1" data-id="PAGE" id="pageNo"><a href="Doc/list?pageNo=<s:property value="key"/>&state=${state}"><s:property value="key"/></a></li>	
						            	</s:if>
						            	<s:else>
				                ...
				                <s:if test="#request.page.showNext != 0">
				               		<li class id="nextPage" data-id="PAGE"><a href="Doc/list?pageNo=<s:property value="#request.page.pageNo +1"/>&state=${state}" >下一页</a></li>
				               		<li class id="endPage" data-id="PAGE"><a href="Doc/list?pageNo=<s:property value="#request.page.totalPage"/>&state=${state}">尾页</a></li>
				                </s:if>
				                <s:else>
...
function t1(obj,TypeTw){
 	 ...
 	 if(parentid!=-1){//如果id==-1，说明没选任何类别
     	 ...
     	 $.ajax({
     		 type:'GET',
     		 dataType:'json',
     		 url:'/dmis/Type/TypeAction/listaByParentId?parentid='+parentid,//parentid=父类的id
     		 success:function(jsonData){
     			 var data = eval(jsonData);
     			 /* if(data == ""){
         			 $('#'+TypeTw).empty();
  ...
  function t2(obj,TypeTh){
 	 ...
 	 if(parentwoid!=-1){
 		 ...
 		 $.ajax({
 			 type:'GET',
 			 dataType:'json',
 			 url:'/dmis/Type/TypeAction/listaaByParentId?parentid='+parentwoid,
 					 success:function(jsonData){
 						 var data=eval(jsonData);
 						 $.each(data,function(i,n)
 								 {
